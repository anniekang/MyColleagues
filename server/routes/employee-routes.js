      ...
      .then( result => {
        if (result.records.length === 0) {
          return session.run(`
            ...
            WITH mgr
            CREATE (new:Employee {id: { ID }, first_name: { First_Name }, last_name: { Last_Name }, photo: { Photo }, job_title: { Job_Title }, job_description: { Job_Description }, email: { Email }, manager_id: { Manager_ID }, linkedin: { LinkedIn }})
        ...
        .then(result => {
          if (result.records.length === 1) {
            return session.run(`
              ...
              SET update.first_name = { First_Name }, update.last_name = { Last_Name }, update.photo = { Photo }, update.job_title = { Job_Title }, update.job_description = { Job_Description }, update.email = { Email }, update.manager_id = { Manager_ID }, update.linkedin = { LinkedIn }
